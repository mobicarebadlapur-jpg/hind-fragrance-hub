...
export const requestOrderCancellationOrRefund = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: order } = await db.from("orders").select("id,order_number,customer_id,total,status,refund_status").eq("id", data.orderId).eq("customer_id", context.userId).maybeSingle();
    if (!order) return { ok: false as const, error: "Order not found." };
    if (["cancelled", "refunded"].includes(order.status) || ["requested", "approved", "paid"].includes(order.refund_status ?? "")) {
      return { ok: false as const, error: "This order already has a cancellation/refund request." };
    }

    if (["created", "payment_pending"].includes(order.status)) {
      const { error } = await db.from("orders").update({ status: "cancelled", refund_status: null, refund_requested_at: null, refund_requested_amount: null, refund_reason: data.reason }).eq("id", order.id).eq("customer_id", context.userId);
    ...
    const { error } = await db.from("orders").update({ refund_status: "requested", refund_requested_at: new Date().toISOString(), refund_requested_amount: order.total, refund_reason: data.reason }).eq("id", order.id).eq("customer_id", context.userId).is("refund_status", null);
    if (error) return { ok: false as const, error: error.message };
...
export const processAdminRefund = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const db = await admin();
    const { data: order } = await db.from("orders").select("id,order_number,customer_id,total,status,refund_status,refund_reason").eq("id", data.orderId).maybeSingle();
    ...
    if (data.decision === "rejected") {
      ...
      const { error } = await db.from("orders").update({ refund_status: "rejected", refund_processed_at: new Date().toISOString(), refund_reason: reason }).eq("id", order.id).eq("refund_status", "requested");
      if (error) return { ok: false as const, error: error.message };
    ...
    const { error: orderError } = await db.from("orders").update({ status: "refunded", refund_status: "paid", refund_processed_at: now, refund_payment_id: refundPaymentId }).eq("id", order.id).eq("refund_status", "requested");
