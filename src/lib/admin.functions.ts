...
export const updateSetting = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    await db.from("app_settings").upsert({ key: data.key, value: data.value as never, updated_at: new Date().toISOString() });
...
export const updateOrderStatus = createServerFn({ method: "POST" })
  ...
  .inputValidator((input: unknown) => z.object({
    ...
    status: z.enum(["created", "payment_pending", "paid", "processing", "shipped", "delivered", "cancelled", "refunded", "returned"]),
  }).parse(input))
  .handler(async ({ data, context }) => {
    if (!(await isAdmin(context.userId))) return { ok: false as const, error: "Forbidden" };
    const db = await admin();
...
export const updatePayoutStatus = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    await db.from("payouts").update({ status: data.status, notes: data.notes ?? null }).eq("id", data.payoutId);
    if (data.status === "paid") {
