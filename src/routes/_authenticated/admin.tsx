...
export const Route = createFileRoute("/_authenticated/admin")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Manage Hind Fragrance partners, products and platform operations." },
    ],
  }),
          ...
          <TabsContent value="partners" className="mt-6">
            <div className="mb-5 flex items-center justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-muted-foreground">Approve pending applications to activate partner referral access.</p>
              </div>
              <Button variant="outline" size="sm" onClick={() => void partnersQuery.refetch()} disabled={partnersQuery.isFetching}>Refresh</Button>
            </div>
            ...
            {partnersQuery.isLoading ? (
              ...
              <div className="space-y-3">
                {partnersQuery.data?.map((partner) => (
                  <div key={partner.id} className="rounded-xl border border-border bg-card p-5">
                    <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                      <div className="min-w-0">
                        <div className="flex flex-wrap items-center gap-3"><span className="font-medium">{partner.partner_code}</span><StatusPill status={partner.status} /></div>
                        <p className="mt-2 text-xs text-muted-foreground">User: {partner.user_id}</p>
                        <p className="mt-1 text-xs text-muted-foreground">Referral: {partner.referral_code} · Joined {new Date(partner.joined_at).toLocaleDateString("en-IN")}</p>
                      </div>
                      <div className="flex flex-wrap gap-2">
                        {partner.status === "pending" && <Button size="sm" onClick={() => void changeStatus(partner.id, "active")} disabled={busyId === partner.id}>{busyId === partner.id ? "Updating…" : "Approve"}</Button>}
                        {partner.status === "active" && <Button size="sm" variant="outline" onClick={() => void changeStatus(partner.id, "suspended")} disabled={busyId === partner.id}>Suspend</Button>}
                        {(partner.status === "pending" || partner.status === "suspended") && <Button size="sm" variant="destructive" onClick={() => void changeStatus(partner.id, "cancelled")} disabled={busyId === partner.id}>Cancel</Button>}
                        {partner.status === "suspended" && <Button size="sm" onClick={() => void changeStatus(partner.id, "active")} disabled={busyId === partner.id}>Reactivate</Button>}
                      </div>
                    </div>
                  </div>
                ))}
          ...
          <TabsContent value="products" className="mt-6 space-y-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-muted-foreground">Manage prices, stock, commission, images and storefront visibility.</p>
              </div>
              <div className="flex gap-2">
                {productForm.id && <Button variant="outline" size="sm" onClick={() => setProductForm(EMPTY_PRODUCT)}>New product</Button>}
                <Button variant="outline" size="sm" onClick={() => void productsQuery.refetch()} disabled={productsQuery.isFetching}>Refresh</Button>
              </div>
            </div>

            <div className="rounded-xl border border-border bg-card p-5">
              <h3 className="font-medium">{productForm.id ? "Edit product" : "Add product"}</h3>
              ...
              <div className="mt-5 flex gap-2"><Button onClick={() => void saveCurrentProduct()} disabled={productBusy}>{productBusy ? "Saving…" : productForm.id ? "Update product" : "Create product"}</Button>{productForm.id && <Button variant="outline" onClick={() => setProductForm(EMPTY_PRODUCT)}>Cancel edit</Button>}</div>
            </div>
            ...
            {productsQuery.isLoading ? (
              ...
              <EmptyState message="No products yet. Add the first product above." />
            ) : (
              <div className="space-y-3">
                  ...
                  <div key={product.id} className="rounded-xl border border-border bg-card p-5">
                    <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                      <div className="flex min-w-0 gap-4">
                        {product.image_url ? <img src={product.image_url} alt="" className="h-16 w-16 rounded-lg object-cover" /> : <div className="h-16 w-16 rounded-lg border bg-muted" />}
                        <div className="min-w-0">
                          <div className="flex flex-wrap items-center gap-2"><span className="font-medium">{product.name}</span><StatusPill status={product.status} /></div>
                          <p className="mt-1 text-xs text-muted-foreground">{product.sku} · {product.category} · Stock {product.stock}</p>
                          <p className="mt-1 text-sm">₹{Number(product.sale_price ?? product.price).toLocaleString("en-IN")} {product.sale_price != null && <span className="ml-2 text-xs text-muted-foreground line-through">₹{Number(product.price).toLocaleString("en-IN")}</span>}</p>
                          <p className="mt-1 text-xs text-muted-foreground">Commission {Number(product.commission_percent ?? 0)}%{product.featured ? " · Featured" : ""}</p>
                        </div>
                      </div>
