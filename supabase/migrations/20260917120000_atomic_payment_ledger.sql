-- Atomic payment confirmation for the current checkout flow.
-- The checkout server creates a pending transaction first; this RPC locks and
-- settles that exact transaction while also locking the order and products.
...
BEGIN
  ...
  IF _order_id IS NULL
     OR _gateway_order_id IS NULL OR btrim(_gateway_order_id) = ''
     OR _payment_id IS NULL OR btrim(_payment_id) = ''
     OR _gateway IS NULL OR btrim(_gateway) = ''
     OR _gateway_payment_id IS NULL OR btrim(_gateway_payment_id) = ''
...
-- Remove the old 5-argument overload so the application cannot accidentally
-- resolve to the obsolete transaction-creating implementation.
DROP FUNCTION IF EXISTS public.confirm_paid_order(uuid, text, text, text, numeric);

REVOKE ALL ON FUNCTION public.confirm_paid_order(uuid, text, text, text, text, numeric, text)
  FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.confirm_paid_order(uuid, text, text, text, text, numeric, text)
  TO authenticated, service_role;

COMMENT ON FUNCTION public.confirm_paid_order(uuid, text, text, text, text, numeric, text)
IS 'Atomically confirms an authenticated customer order by validating the canonical amount, locking the existing payment transaction and products, deducting stock, settling the payment ledger, and marking the order paid.';
